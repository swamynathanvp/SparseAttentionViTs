...
# from transformers.hubconf import *
...
class BigBirdBlockSparseAttention(nn.Module):
    def __init__(self, config, attentions,seed=None):
        super().__init__()
        ...
        ####
        self.use_random_attention = "Random" in attentions
        self.use_window_attention = "Window" in attentions
        self.use_Global_attention = "Global" in attentions

        if(self.use_random_attention):
            print("Training with random attention")
        if(self.use_window_attention):
            print("Training with window attention")
            
        if(self.use_Global_attention):
            print("Training with global attention")
...
###
        ...
        # return torch.zeros_like(context_layer)
    ...
    def bigbird_block_sparse_attention(
        ...
        ####### Calculate it after something you know is gonna be found
        # n1,n2,_,n3,n4 = first_context_layer.shape
        ...
        # n1,n2,_,n3,n4 = second_context_layer.shape
        # device = second_context_layer.device
        # first_context_layer = torch.zeros(n1, n2, 1, n3, n4).to(device)
        ...
        ############################################################################
        # print(first_context_layer.shape)
        # print(second_context_layer.shape)
        # print(context_layer.shape)
        # print(second_last_context_layer.shape)
        # print(last_context_layer.shape)
        ...
        # first_context_layer = [16, 16, 1, 10, 64]
        # second_context_layer = [16, 16, 1, 10, 64]
        # context_layer=[16, 16,49,10, 64]
        # second_last_context_layer=[16, 16, 1, 10, 64]
        # last_context_layer = [16, 16, 1, 10, 64]
        # device = first_context_layer.device
        # device = second_context_layer.device
        # n1,n2,_,n3,n4 = [16, 16, 1, 10, 64]
        # # n1,n2,_,n3,n4 = second_context_layer.shape
        # # # n1,n2,_,n3,n4 = first_context_layer.shape
        # first_context_layer = torch.zeros(n1, n2, 1, n3, n4).to(device)
        # second_context_layer =      torch.zeros(n1, n2, 1, n3, n4).to(device)
        ...
        # context_layer = first_context_layer
        context_layer = context_layer.view((bsz, n_heads, from_seq_len, -1)) * from_mask
        context_layer = torch.transpose(context_layer, 1, 2)
        # assert (context_layer == 0).all() == True
    ...
    def _bigbird_block_rand_mask_with_head(
        ...
        for plan_idx in range(max_plan_idx + 1):
            ...
            for blk_rw_idx in range(from_start_block_id, plan_block_length[plan_idx]):
                for h in range(num_heads):
                    rand_attn[h][blk_rw_idx, rnd_r_cnt: ] = self._get_single_block_row_attention(
                        block_id=blk_rw_idx,
                        to_start_block_id=to_start_block_id,
                        to_end_block_id=plan_block_length[plan_idx],
                        num_rand_blocks=plan_num_rand_blocks[plan_idx],
